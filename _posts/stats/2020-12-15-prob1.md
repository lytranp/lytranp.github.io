...
+If A and B are independent, P(A $|$ B) = P(A). So P(A $\cap$ B) = P(A)*P(B)

+If A and B are not independent, P(A$|$B) != P(A).So, P(A $\cap$ B) = P(A|B) * P(B)
...
In conditional probability,  we have to compute P(A|B). Conversely, we need to know P(B|A) if we know conditional probability P(A|B). Bayes theorem now comes into the picture. 
